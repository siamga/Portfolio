...
class company():
    ...
    def start(self):
        ...
        self.df.columns = self.df.columns + self.df.loc[0,:].astype('float64').replace(np.nan,0).astype('int64').astype('str')


        self.df.drop(0,axis=0,inplace=True)
        self.df.rename(columns={'구분0':'구분'},inplace=True)
    ...
    def show_company(self,start):
        ...
        df1 = self.df.set_index('구분')[self.df.columns[self.df.columns.str.contains('{}'.format(self.start))]]
        df2 = self.df.set_index('구분')[self.df.columns[self.df.columns.str.contains('{}'.format(self.start+1))]]
        df3 = self.df.set_index('구분')[self.df.columns[self.df.columns.str.contains('{}'.format(self.start+2))]]

        self.df = pd.concat([df1,df2,df3],axis=1).dropna().reset_index()
        ...
        for i in self.df.columns:
            for j in self.df.index:
                if self.df[i][j] == '999999999' or self.df[i][j] == '0':
                    self.df.drop(j,axis=0,inplace=True)
